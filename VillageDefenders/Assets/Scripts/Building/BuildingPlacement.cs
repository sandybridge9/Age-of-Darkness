...
public class BuildingPlacement : MonoBehaviour
{
    ...
    {
        hasPlaced = false;
        currentBuilding = Instantiate<Building>(b);//((Building)Instantiate(b)).transform;
    ...
    {   
        ...
        //Vector3 rayStartingPosition = ray.origin;

        if (Physics.Raycast(ray, out hitInfo, 10000f, layerToHit))
        {
            currentBuilding.transform.position = hitInfo.point;
            //currentBuilding.transform.rotation = Quaternion.FromToRotation(/*currentBuilding.transform.rotation.eulerAngles*/Vector3.up, hitInfo.normal);
